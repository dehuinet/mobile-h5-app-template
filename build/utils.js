...
const {eslintFormatterBuild, extractBuildArg, getGitInfo, manifestGenerator, preloadPluginBuild, printStats, stylelintFormatterBuild} = require('webpack-minxing-utils');
...
exports.cssLoaders = function(options){
    ...
    const px2remLoader = {
        ...
        options: {
            exclude: [
                'node_modules/vant',
                'node_modules/mx-vue-mobile-components'
            ],
            remUnit: 750 / 10, // 设计稿宽度 除以 10；若仅画设计稿内某区域里的内容时，则是设计稿中被绘制区域的宽度 除以 10。
            baseDpr: 1
        }
    };
    const postcssLoader = {
        ...
        return ['css-hot-loader', {
            ...
            */
            ...
            options: {
                ...
                */
                publicPath: '../'.repeat(assetsSubDirectory.split('/').length + 1),
                // 仅在产品模式下，`extract`才会被设置为`true`。
                ...options.extract ?
                    {} :
                    {// 为了兼容 IE 9，并没有使用 ['vue-style-loader']。
                        hmr: true,
                        reloadAll: true
                    }
            }
    ...
    return {
        ...
        less: generateLoaders('less', {
            modifyVars: {
                hack: `true; @import "${exports.resolve('./src/assets/vant/theme.less')}";`
            }
        }),
...
exports.eslintFormatter = function(results){
    const config = require('../config');
    return eslintFormatterBuild(path.join(config.build.assetsRoot, '..'))(results);
};
exports.stylelintFormatter = function(result){
    const config = require('../config');
    return stylelintFormatterBuild(path.join(config.build.assetsRoot, '..'))(result);
};
